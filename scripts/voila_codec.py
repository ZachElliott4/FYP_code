...
def main() -> None:
    ...
    print(f"Audio length {orig_len/16_000:.2f} s")

    # 3) Tokenise
    with torch.no_grad():
        enc_out = codec.encode(
            fx(wav, sampling_rate=16_000, return_tensors="pt")["input_values"].to(args.device)
        )
    ...
    print(f"Tokenised into {len(tokens)} IDs  (codebooks={n_q})")

    # 4) Arithmetic coding with LM
    ...
    for i in range(1, len(ids)):
        ...
        if i <= args.trace_steps:
            top = probs.argsort()[-8:][::-1]
            print("step", i, "→ chosen id", ids[i],
                  "| top-1 prob =", probs[ids[i]])
        enc.encode(ids[i],
                   constriction.stream.model.Categorical(probs.astype(np.float32),
                                                         perfect=False))
    bitstream = enc.get_compressed()
    lm_bits_per_token = 8 * len(bitstream) / len(tokens)
    print(f"Compressed with LM → {len(bitstream)} bytes "
          f"({lm_bits_per_token:.2f} bits/token)")

    # 5) Uniform baseline
    vocab = lm.config.vocab_size
    uniform_probs = np.ones(vocab, dtype=np.float32) / vocab
    enc_u = constriction.stream.queue.RangeEncoder()
    for t in tokens:
        enc_u.encode(t,
                     constriction.stream.model.Categorical(uniform_probs,
                                                           perfect=False))
    bitstream_u = enc_u.get_compressed()
    uniform_bpt = 8 * len(bitstream_u) / len(tokens)
    print(f"Uniform coding → {len(bitstream_u)} bytes "
          f"({uniform_bpt:.2f} bits/token)")
    ...
    for _ in range(1, len(ids)):
        ...
        probs = torch.softmax(lm(ctx).logits[0, -1].float(), -1).cpu().numpy()
        decoded.append(dec.decode(constriction.stream.model.Categorical(
    ...
    stats = {
        "n_tokens": len(tokens),
        "bits_per_token_lm": lm_bits_per_token,
        "bits_per_token_uniform": uniform_bpt,
        "compression_gain_bits": uniform_bpt - lm_bits_per_token,
    }
    (args.outdir / "stats.json").write_text(json.dumps(stats, indent=2))
    print("Stats written to", args.outdir / "stats.json")
